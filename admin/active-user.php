...
<div class="row mb-2">
<div class="col-sm-6">
<h1 class="m-0">Active User Dashboard</h1>
</div><!-- /.col -->
<div class="col-sm-6">
...
<tbody>
<?php
$sql = "SELECT * FROM admin INNER JOIN staff_details ON staff_details.staff_no=admin.staff_no WHERE status=0 ";
...
<?php }?>
<?php
$sql = "SELECT * FROM staff_user INNER JOIN staff_details ON staff_details.staff_no=staff_user.staff_no WHERE status=0 ";
$query = $conn->prepare($sql);
$query->execute();
$fetch = $query->fetchAll();
foreach ($fetch as $key => $value) { ?>
